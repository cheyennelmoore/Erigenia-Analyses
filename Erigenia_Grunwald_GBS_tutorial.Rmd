...
library(ade4)
```
Below: useful for upgma tree, not for hierfstat work.
...
below: hierfstat work
```{r}
Erigenia.stats<-basic.stats(Erigenia_hierfstat,diploid=TRUE,digits=4)
```
```{r}
Erigenia.stats.perloc.txt <- print(Erigenia.stats,perloc)
```
