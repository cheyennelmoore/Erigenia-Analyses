...
```
```{r}
scatter.dapc(Erigenia.dapc) 
```
Can add more arguments  to above code and change other features
...
compoplot(s_vib.dapc,col = colors, posi = 'bottom')
```


#STRUCTURE-like plot
```{r make DAPC results}
dapc.results <- as.data.frame(s_vib.dapc$posterior)
dapc.results$pop <- pop(s_vib_gen)
dapc.results$indNames <- rownames(dapc.results)
```

##ggplot
```{r getting dapc ready for ggplot}
dapc.results <- melt(dapc.results)
colnames(dapc.results) <- c("Original_Pop","Sample","Assigned_Pop","Posterior_membership_probability")
```
